...
void IRCache::UnregisterJit() {
    ...
    if (--s.jit_count == 0) {
        ...
        s.total_bytes = 0;
        IRCacheStats::ir_cache_entries.store(0, std::memory_order_relaxed);
        IRCacheStats::ir_cache_bytes.store(0, std::memory_order_relaxed);
    }
}
...
void IRCache::Store(std::uint64_t descriptor_value, const IR::Block& block,
    ...
    IRCacheStats::ir_stores.fetch_add(1, std::memory_order_relaxed);
    IRCacheStats::ir_cache_entries.store(s.map.size(), std::memory_order_relaxed);
    IRCacheStats::ir_cache_bytes.store(s.total_bytes, std::memory_order_relaxed);
}
