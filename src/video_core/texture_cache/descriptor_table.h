...
public:
    ...
    [[nodiscard]] std::pair<T, bool> Read(Tegra::MemoryManager const& gpu_memory, u32 index) noexcept {
        ...
        static thread_local u64 total_reads{};
        static thread_local u64 fast_path_hits{};
        ++total_reads;
        ...
        if ((total_reads & 0xffff) == 0) [[unlikely]] {
            LOG_INFO(HW_GPU, "SerialCuts descriptor diag: fast_path_hits={} total_reads={}",
                     fast_path_hits, total_reads);
        }
