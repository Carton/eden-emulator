...
void TextureCache<P>::QueueAsyncDecode(Image& image, ImageId image_id) {
    ...
    // Zero-fill the image immediately: decoded contents are uploaded in a
    // later frame by TickAsyncDecode, but draws can sample the image in
    // between and would read never-written VRAM (stale garbage tiles, e.g.
    // on loading screens). Sampling the decode window reads black instead.
    {
