...
### Serial ???? ? PrepareDraw / ConfigureTail / draws-per-frame???????

???????????/?/?????????????????
PrepareDraw ???? ns ??? SerialDraw ??? 5000 calls ???prologue ?
prepare_start ? pipeline ???????? FlushPendingDraw??lock_setup ?? B/T
????SetEngine?Reset?pending uniform?resolve?configure_tail ?????
dynamic_query ??????/??/XFB?emit ? draw_func?null_pipeline ? tail_rejected
??????????????????? calls ???????????????
????????? prepare_avg_ns ??????????????? gpu.TickWork?
?? 3.08us ????????????? ConfigureTail ???/???????????

ConfigureTail ????? ConfigureImpl ?? tail ???Resolve-only ????
?????? job_bindings ??????? 5000 calls ?? ConfigureTail diag?
uniform_apply ? ApplyJobUniformBindings?????????texture_apply ??
ApplyGraphicsTextureBufferBindings ???????other ? XFB render-pass ???
storage binding ???bindings_upload_descriptors ?? SetUniformBuffersState?
UpdateGraphicsBuffers?geometry/stage buffers?????render targets/feedback?
ConfigureDraw??? gate off ???????? uniform ????? bindings ??
??????????????????? tail ??????job-bindings ??????????

SerialFrame ? 500 TickFrame ???? frames/draws/avg/max?draw_counter ? dispatch
??????? FlushWork/FlushCommands ???????????? draw ????
Draw + DrawIndirect ???????TickFrame ???????? API ???????/??
? token ???indirect ? draw ????????? Vulkan ? draw ?????? loading?
?????????? draws/frames ?????????? diag????/???
...
????????FlushWork ??FlushCaching ??UpdateGraphicsBuffers ?? geometry
????UpdateGraphicsBuffers ????????? PrepareDraw ??? ConfigureTail
other/end ????? serial ?? draw ?? 14 ??? 10+4??job-bindings ????
?????? apply ?????????????????????/?/??/????

SerialDraw prologue diag ? 5000 calls ? flush_work_ns/flush_caching_ns/
pipeline_lookup_ns????????????????? prologue???????
FlushWork ?????? scope ???????????????

UpdateGraphicsBuffers diag?TLS?? Vulkan tail ?????? 5000 calls?
geometry_setup_ns ???? tail other ????? index/vertex/XFB ???????
SetUniformBuffersState??????uniform_storage_texture_indirect_retry_ns ???
????????????? stage ??? uniform/storage/texture?indirect ???
?????? geometry??passes ??? BufferOperations ???????? stage ?
????????????????/????????

ConfigureTail diag ?? bindings_upload_descriptors_ns???? update_graphics_ns
? post_update_bindings_descriptors_ns????? BindHostGeometryBuffers ???/???
? stage host buffer ??/???????render targets/feedback ? ConfigureDraw?
???? CBPG graphics ?? BindHostGraphicsUniformBuffers??? post_update?
?? UpdateUniformBuffers?????/FindBuffer????????? interleaved uniform
? storage???? post_update ??????????/?? fast paths ? CBPG ??????
UpdateGraphicsBuffers ????/??????? end ???????? post_update??
configure_tail ????????????????????????

???buffer_cache/buffer_cache.h?vk_graphics_pipeline.cpp?vk_rasterizer.cpp/.h?
????????????????????????? diag????????diff --check
...
### Serial step 2 ? post-update ???????????????

? EDEN_SERIAL_PROLOGUE_FAST ???? "1" ? token_mode==Off ??????????
token/fallback ? prologue ?????????????? draw getenv??????

FlushCaching?InvalidationAccumulator::HasPending ???? InvalidateAll ? start_address>0
???MemoryManager ??? const ?????serial GPU producer ??????????
FlushCaching????????????/????????????????????????
?? accumulator ?? sentinel ????????????????????
FlushWork?? CHECK_MASK ?? PrepareDraw ?? ++draw_counter??????? FlushWork?
?????????????Android mask3/??mask7 ???????????????
? u32 ?????/dispatch ????????DispatchWork/Flush ?????????? mask
??? FlushWork ?????? SerialDraw prologue fast diag ? gate?eligible_calls?
?? skips?pipeline_lookup_skips ??0?
Pipeline lookup ??????CurrentGraphicsPipeline ???? shader invalidation?HLE?
topology/channel ?????? generation ????? RefreshStages?key.Refresh?Next/
BuiltPipeline ?????? parser warm-hit ?????? serial ?????????

ConfigureTail ???????????????host_geometry_ns?UpdateGraphicsBuffers
end ? BindHostGeometryBuffers ????????????host_uniform_ns?? stage
BindHostGraphicsUniformBuffers ????????? CBPG ????stage_remaining_ns
?descriptor queue Acquire?host storage/texture?PushImageDescriptors/render-area ???
? stage ???????? uniform ????final_emit_ns?PostCopyBarrier?render-target/
feedback?????? ConfigureDraw ??????/????????????????
???????14??16+2*??stage?????????uniform??????????
14???????? A/B??????????????????TLS ???????/???

?????buffer_cache.h?buffer_cache_base.h??? GraphicsUpdateDiag ?????
...
### Serial step 3 ? ???????? uniform ?????????

A ????????????????? resolve ?????ConfigureImpl Resolve ??
?? pipeline shader ????CB ???? guest texture handles?? secondary??sampler
???TIC/TSC ? texture cache ??/??/?????? SynchronizeDescriptors/FillImageViews
???????storage binding ??? tail ????? resolve ??????DrawContext ?
uniform binding revisions ??? guest ????????????????????????
??? generation???????????/??/????????? <=100ns/draw?
????? ResolveBindset unchanged/uniform_only_changed ?????????????

????1?classic SynchronizeBuffer ? ForEachUploadRange ? memory_tracker_base
ForEachModifiedRange(Type::CPU,true) ?????????? return true?? memcmp???
???? UploadMemory?CBPG graphics ?? stream ??? BindMappedUniformBuffer ?
?????epoch ? guest???? CPU-modified ??????? GPU-modified??? CPU?
430ns ???????????? CPU-dirty ?????????? stream ???????
stream ???????stage/index/size/?????????? memcpy?

B ? UniformSync diag??65536? graphics uniform binding ???syncs_total?
stream_copies/bytes?tracker_clean_no_upload?tracker_dirty_uploaded?compare_equal_no_upload
??0??????????????compare_equal_uploaded/compare_differ_uploaded/
bytes_compared ???? EDEN_UNIFORM_STATS ?????? shadow ??????????
?????0???????????????/size ????? comparison_baselines?
bytes_compared ? memcmp ???????????????????? shadow ?????
?????????????????????????TLS ?????????????
?????????????????????????? <=100ns???? shadow ????
???/?????????100ns???????????????

????2?????? resolver job_bindings_enabled ? BindUniformInput/DisableUniformInput
?????CaptureUniformInputs ?? job ctx??? serial ??? legacy cache?????
?????????????????? job bindings????????????? CB ??
? TIC/TSC ?????????????? resolve fast-path ???????????????
